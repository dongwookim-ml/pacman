...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p] = self.beliefs[p] * emissionModel[t]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        garbage = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                garbage[newPos] += prob * self.beliefs[oldPos]
        #print "BELEIFS IS " + beliefs.__str__()
        #print "GARBAGE IS " + garbage.__str__()
        garbage.normalize()
        self.beliefs = garbage
    def getBeliefDistribution(self):
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for x in range(0, self.numParticles-1):
            self.parcles.append(self.legalPositions[x % len(self.legalPositions)])
    def observe(self, observation, gameState):
        """
    ...
    def elapseTime(self, gameState):
        ...
        """
        garbage = []
        for oldPos in self.parcles:
            #print oldPos
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #print newPosDist
            garbage.append(util.sample(newPosDist))
            #print garbage
        #print garbage
        self.parcles = garbage
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        jun = itertools.product(self.legalPositions, repeat = self.numGhosts)
        #Tx = jun.next()
        #Eprint x
        junk = []
        while True:
            try:
                junk.append(jun.next())
        ...
        #print junk
        for k in range (0, self.numParticles - 1):
            particles.append(junk[k % len(junk)])
